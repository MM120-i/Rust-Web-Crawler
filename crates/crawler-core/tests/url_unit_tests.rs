use crawler_core::url::{normalize_for_key, resolve_dot_segments, AdmissionError};
use url::Url;

#[test]
fn crawl_key_strips_fragment() {
    let url = Url::parse("https://example.com/page#section").unwrap();
    let key = normalize_for_key(&url).unwrap();
...
fn crawl_key_preserves_duplicate_query_keys() {
    let url = Url::parse("https://example.com/?b=1&b=2").unwrap();
    let key = normalize_for_key(&url).unwrap();
