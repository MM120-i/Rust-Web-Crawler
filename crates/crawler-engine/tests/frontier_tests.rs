...
fn make_admitted(path: &str, depth: u32) -> AdmittedUrl {
    ...
    let crawl_key = crawler_core::url::normalize_for_key(&url).unwrap();
    AdmittedUrl {
        crawl_key,
        url,
        depth,
...
fn enqueue_returns_admitted_for_new_url() {
    let mut frontier = Frontier::new(CrawlJobId(1), 10);
    let admitted = make_admitted("/page1", 0);

    let result = frontier.enqueue(admitted);
...
fn enqueue_returns_duplicate_for_same_url() {
    let mut frontier = Frontier::new(CrawlJobId(1), 10);
    let a = make_admitted("/page1", 0);
    let b = make_admitted("/page1", 1);

    frontier.enqueue(a);
    let result = frontier.enqueue(b);
...
fn enqueue_returns_at_capacity_when_max_pages_reached() {
    ...
    let result = frontier.enqueue(make_admitted("/c", 0));
...
fn mark_complete_removes_from_in_flight() {
    let mut frontier = Frontier::new(CrawlJobId(1), 10);
    let admitted = make_admitted("/page1", 0);
    let key = admitted.crawl_key.clone();
...
fn stats_tracks_counts() {
    let mut frontier = Frontier::new(CrawlJobId(1), 10);
    let a = make_admitted("/a", 0);
    let b = make_admitted("/b", 0);
    let key_a = a.crawl_key.clone();
...
fn duplicate_detection_persists_after_complete() {
    ...
    let result = frontier.enqueue(make_admitted("/page1", 0));
    assert!(matches!(result, EnqueueResult::Duplicate));
}
...
fn different_urls_are_not_duplicates() {
    let mut frontier = Frontier::new(CrawlJobId(1), 10);
    frontier.enqueue(make_admitted("/a", 0));
    let result = frontier.enqueue(make_admitted("/b", 0));
    assert!(matches!(result, EnqueueResult::Admitted { .. }));
}

#[test]
fn empty_frontier_is_empty() {
    let frontier = Frontier::new(CrawlJobId(1), 10);
